...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post("/auth/login", { email, role: "ADMIN" });
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <h2 className="font-heading text-4xl tracking-[-0.04em]">Admin Sign In</h2>
        <p className="text-sm leading-6 text-muted-foreground">Enter your admin email to receive a verification code.</p>
      </div>
      ...
      <div className="border-t border-border/70 pt-4 text-center">
        <Link href="/student-login" className="text-sm text-primary hover:underline">
          I am a student
        </Link>
      </div>
    </div>
