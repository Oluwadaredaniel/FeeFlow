      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">{student.matric_no} • {student.department} • Level {student.level}</p>
          </div>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => updateStatus("DEFERRED")} className="text-xs">
            <UserX className="mr-2 h-3 w-3" /> Defer
          </Button>
          <Button variant="outline" onClick={() => updateStatus("GRADUATED")} className="text-xs">
            <GraduationCap className="mr-2 h-3 w-3" /> Graduate
          </Button>
          <Button variant="destructive" onClick={() => updateStatus("INACTIVE")} className="text-xs">
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent className="grid grid-cols-3 gap-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-lg font-bold text-green-600">{formatNaira(student.total_paid)}</p>
            </div>
            <div className="space-y-1">
              <p className="text-xs text-muted-foreground">Balance</p>
              <p className="text-lg font-bold text-red-600">{formatNaira(student.total_owed - student.total_paid)}</p>
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          <div className="space-y-6">
            {fees.length === 0 ? (
              ...
              fees.map((fee) => (
                <div key={fee.id} className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 border rounded-lg">
                  <div className="space-y-1">
